...
import type { PickerPlayer, SquadDraft } from "@/lib/drafts/types";
    ...
    >
      <div className="flex h-full w-full max-w-5xl flex-col overflow-hidden bg-background shadow-2xl md:my-4 md:h-auto md:max-h-[95vh] md:rounded-2xl md:border">
        ...
        <div className="grid flex-1 grid-cols-1 overflow-y-auto md:grid-cols-2">
          {[
            { side: "L", draft: left, stats: leftStats, valid: leftValid, value: leftId, setValue: setLeftId, onlyHere: onlyLeft },
            { side: "R", draft: right, stats: rightStats, valid: rightValid, value: rightId, setValue: setRightId, onlyHere: onlyRight },
          ].map((col) => (
            <div
              key={col.side}
              className={cn(
